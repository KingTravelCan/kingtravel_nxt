  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      <button
        ...
        disabled={loading}
        className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl border border-slate-200 bg-white text-xs text-left outline-none focus:border-[#004B39] disabled:opacity-50"
      >
        ...
        <div className="absolute z-50 w-full mt-2 bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden flex flex-col max-h-[300px]">
          <div className="overflow-y-auto flex-1 py-1">
            {categories.length === 0 ? (
              ...
              categories.map((cat) => (
                <div
                  key={cat}
                  className={`flex items-center justify-between px-3 py-2 text-xs cursor-pointer hover:bg-slate-50 transition-colors group ${
                    value === cat ? 'bg-emerald-50 text-emerald-800 font-semibold' : 'text-slate-700'
                  }`}
                  onClick={() => {
                    onChange(cat);
                    setIsOpen(false);
          ...
          <div className="border-t border-slate-100 p-2 bg-slate-50 flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-2 text-xs border border-slate-200 rounded-lg outline-none focus:border-[#004B39]"
            />
            <button
              type="button"
              onClick={handleAddCategory}
              disabled={saving || !newCategory.trim()}
              className="px-3 py-2 bg-[#004B39] text-white rounded-lg hover:bg-[#003829] disabled:opacity-50 transition-colors flex items-center justify-center"
            >
