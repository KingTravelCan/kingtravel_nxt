  ...
  return (
    <div className="fixed inset-0 z-[99999] bg-slate-950/80 backdrop-blur-sm flex justify-center items-center p-4 animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[92vh]">
        {/* Modal Header */}
        <div className="bg-[#004B39] text-white p-5 flex justify-between items-center">
          <div>
            ...
            <h2 className="text-xl font-bold font-serif text-white mt-0.5">
              Detail Page for: <span className="underline text-[#DB9E30]">{pkg.title || "Untitled Package"}</span>
            </h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="bg-slate-100 border-b border-slate-200 px-5 pt-3 flex gap-2 overflow-x-auto text-xs font-bold">
          <button
            ...
            className={`px-4 py-2.5 rounded-t-xl transition-colors flex items-center gap-1.5 cursor-pointer ${
              activeTab === "overview"
                ? "bg-white text-[#004B39] border-t-2 border-[#004B39] shadow-sm font-extrabold"
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1 bg-slate-50 space-y-6">
          ...
          {activeTab === "overview" && (
            <div className="space-y-5">
              ...
              <div className="bg-[#004B39] text-white p-5 rounded-2xl relative overflow-hidden shadow-lg border border-emerald-950">
                <div className="text-[9px] font-extrabold uppercase text-[#DB9E30] tracking-widest mb-1.5 flex items-center gap-1">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                  ...
                  <div className="bg-[#00382B] border-2 border-dashed border-[#DB9E30] rounded-xl p-3 text-center min-w-[170px] shrink-0">
                    <span className="text-[9px] font-extrabold uppercase text-[#DB9E30] block">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 bg-white p-4 rounded-xl border border-slate-200">
                <div>
                  <label className="block text-[10px] font-bold text-slate-700 mb-1">
                    <Globe className="w-3 h-3 inline text-emerald-600 mr-1" /> DEPARTURE
                  </label>
                  <input
                    type="text"
                    value={departure}
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-700 mb-1">
                    <Globe className="w-3 h-3 inline text-emerald-600 mr-1" /> DESTINATION
                  </label>
                  <input
                    type="text"
                    value={destination}
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-700 mb-1">
                    <DollarSign className="w-3 h-3 inline text-emerald-600 mr-1" /> CURRENCY CODE
                  </label>
                  <input
                    type="text"
                    value={currencyCode}
              ...
              <div className="bg-white p-4 rounded-xl border border-slate-200">
                <label className="block text-xs font-bold text-[#004B39] mb-1">
                ...
                <textarea
                  ...
                  className="w-full bg-slate-50 border border-slate-300 rounded-lg p-3 text-xs font-mono text-slate-800 leading-relaxed focus:ring-2 focus:ring-[#004B39] focus:outline-none"
                />
              </div>
            </div>
          )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="bg-white p-4 rounded-xl border border-slate-200 flex flex-col">
                <label className="block text-xs font-bold text-emerald-800 mb-1 flex items-center gap-1">
                  <Check className="w-4 h-4 text-emerald-600" /> ELIGIBILITY REQUIREMENTS (1 ITEM PER LINE)
                </label>
                <p className="text-[10px] text-slate-500 mb-2">
                ...
                <textarea
                  ...
                  className="w-full bg-slate-50 border border-slate-300 rounded-lg p-3 text-xs text-slate-800 leading-relaxed flex-1 focus:ring-2 focus:ring-emerald-600 focus:outline-none"
                />
              </div>
            </div>
          )}
        ...
        <div className="bg-white border-t border-slate-200 p-4 flex justify-between items-center">
          ...
          <button
            type="button"
            onClick={handleSave}
            className="bg-[#004B39] hover:bg-[#00382B] text-white font-extrabold px-6 py-2.5 rounded-xl text-xs shadow-lg transition-all cursor-pointer"
          >
            Save & Apply Changes
          </button>
        </div>
