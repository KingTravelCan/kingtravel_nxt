...
export function Switch({ id, checked, onChange, disabled = false, className = '' }: SwitchProps) {
  return (
    <button
      ...
      } ${checked ? 'bg-[#004B39]' : 'bg-slate-300'} ${className}`}
    >
      <span
...
export function Field({ children, orientation = 'horizontal', className = '', 'data-disabled': dataDisabled }: FieldProps) {
  return (
    <div
      ...
      className={`flex ${orientation === 'horizontal' ? 'items-center gap-3' : 'flex-col gap-1.5'} ${
        dataDisabled ? 'opacity-50 pointer-events-none' : ''
      } ${className}`}
    >
      {children}
    </div>
