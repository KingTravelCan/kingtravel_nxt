  ...
  return (
    <div className="fixed inset-0 z-[99999] bg-slate-950/70 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 max-w-md w-full p-7 text-center relative flex flex-col items-center gap-4 animate-in zoom-in-95 duration-200">
        ...
        <div className="w-20 h-20 rounded-full bg-emerald-50 text-[#004B39] border-4 border-emerald-100 flex items-center justify-center shadow-lg transform transition-transform hover:scale-105">
          <CheckCircle2 className="w-10 h-10 text-[#004B39]" />
        </div>
        ...
        {referenceNumber && (
          <div className="bg-emerald-50/80 border border-emerald-200/80 px-4 py-2 rounded-2xl w-full flex items-center justify-between text-xs font-bold text-[#004B39]">
        ...
        <button
          type="button"
          onClick={onClose}
          className="w-full mt-2 py-3.5 bg-[#004B39] hover:bg-[#00382B] text-white rounded-2xl text-xs font-extrabold tracking-wider uppercase transition-all shadow-md hover:shadow-lg cursor-pointer border-none"
        >
          Got it, Thanks!
        </button>
      </div>
