  ...
  useEffect(() => {
    getFormsSettings().then((res) => {
      if (res && res.formsData && res.formsData.contact) {
        setFormConfig(res.formsData.contact);
      }
    });
  }, []);
...
export default function ContactPage() {
  ...
  const handledTypes = ["Contact Info Cards", "Contact Bar", "Contact Form", "Contact Form + Maps", "Contact Maps", "Google Maps"];
    ...
    <main className="bg-[#f2f5f3] min-h-screen">
      ...
      <section className="max-w-7xl py-12 mx-auto px-4">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
          <ContactFormSection data={formSec?.data} />
